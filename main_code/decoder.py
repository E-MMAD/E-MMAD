...
CausalLMOutputWithCrossAttentions = modeling_outputs.CausalLMOutputWithCrossAttentions
SequenceClassifierOutputWithPast = modeling_outputs.SequenceClassifierOutputWithPast
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.wte = nn.Embedding(2048, config.n_embd)
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.enc = Encoder('defaults.yaml')

        self.transformer = GPT2Model(config)


        self.lm_head = nn.Linear(config.n_embd, 2048)

        self.new_lm_head = nn.Linear(768, 30000)
    ...
    def forward(
        ...
        enc_output = self.enc(video_feature,frames,title_id,len_title,struct_words_id,len_struct_word,caption_id,len_caption)
        #m4c_output = self.trans_encoder(m4c_output)
        ...
        lm_logits = lm_logits[:,video_feature.size(1)+title_id.size(1)+struct_words_id.size(1):,:]
        if not test:
            labels = torch.cat([caption_id],dim=1)
        loss = None
